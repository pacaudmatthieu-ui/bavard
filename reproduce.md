...
**La mise en forme s'adapte à l'app.** Au moment où vous appuyez sur le raccourci, Bavard regarde quelle application est devant et choisit un mode : **E-mail** (Mail, Gmail, Spark, Outlook), **Message court** (Slack, Messages, WhatsApp), **Technique** (terminal, VS Code, Cursor), **Notes** (Notes, Notion, Obsidian), **Texte standard** ailleurs. Deux endroits pour le changer. **Sur la pastille**, juste sous le mot « Bavard » : six pastilles cliquables (Auto, Mail, Court, Tech, Notes, Texte). Celle qui est pleine est votre choix ; quand « Auto » est actif, le mode détecté pour l'app devant vous est entouré d'un liseré. Un clic pendant que vous parlez s'applique **à la dictée en cours**. La pastille n'apparaissant que pendant la dictée, en mode `hold` (touches maintenues) vous ne pourrez pas cliquer — passez par le menu. Et menu 🎙️ → **Mode de dictée** montre lui aussi ce qui s'appliquerait ici et permet d'épingler. Pour créer vos propres modes, un fichier `~/Documents/Bavard/modes.yaml` ; chaque mode y accepte `label`, `prompt`, `apps`, `titles`, `min_words`, `one_block`.
...
- **Le mauvais mode s'applique** → cliquez la bonne pastille sur l'encart pendant que vous parlez, c'est pris en compte immédiatement. Sinon menu 🎙️ → Mode de dictée : le premier élément indique ce que l'automatique choisirait pour l'app devant vous. Pour une app non reconnue, ajoutez son nom dans `~/Documents/Bavard/modes.yaml` (`email: {apps: [nom de l'app]}`)
